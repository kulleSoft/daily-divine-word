...
const tabs: { id: Tab; icon: typeof BookOpen; labelKey: TranslationKey }[] = [
  { id: 'verses', icon: BookOpen, labelKey: 'verses' },
...
export function BottomNav({ activeTab, onTabChange, t }: BottomNavProps) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-card/95 backdrop-blur-lg border-t border-border safe-area-bottom z-50">
      <div className="flex items-center justify-evenly h-16 max-w-md mx-auto px-2">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={cn(
                "flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-xl transition-all duration-300 flex-1",
                isActive 
                  ? "text-primary" 
